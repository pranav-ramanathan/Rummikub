...
class TileSet:
    ...
    def _create_standard_set(self):
        """Create the standard 106 tile set."""
    ...
    @staticmethod
    def encode_hand(hand: List[Tile], max_size: int = 14) -> List[int]:
        """Encode a hand of tiles to integer list.
        
        Args:
            hand: List of tiles in hand
            max_size: Maximum hand size (for padding)
            
        Returns:
            List of encoded tiles, padded with -1
        """
        encoded = [tile.encode() for tile in hand]
        # Pad with -1 to max_size
        encoded.extend([-1] * (max_size - len(encoded)))
        return encoded[:max_size]
    
    @staticmethod
    def decode_hand(encoded: List[int]) -> List[Tile]:
        """Decode integer list to hand of tiles.
        
        Args:
            encoded: List of encoded tiles (with -1 for empty slots)
            
        Returns:
            List of tiles (excluding empty slots)
        """
        return [Tile.decode(code) for code in encoded if code >= 0]
